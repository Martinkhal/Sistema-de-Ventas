Use Master
Go
If(db_id('DB_A42772_demoPractica')Is Not Null)
Drop DataBase DB_A42772_demoPractica
Create DataBase DB_A42772_demoPractica
Go

Use DB_A42772_demoPractica
Go
...
Insert Cliente Values('00000000','sin datos','sin datos','sin datos','000000000')
Go
...
Insert Categoria Values('Plomeria'),('Herramientas'),('Electricidad'),('Construccion')
Go
...
Create Proc ConsultaTotalVentaDia
(
@FechaVenta Date,
@Mensaje Varchar(50) Output
)
As Begin
Declare @VentaTotal Varchar(20)
		Set @VentaTotal = (select sum(Total) as VentaTotal from Venta Where FechaVenta=@FechaVenta)
		Set @Mensaje='La venta total del dia es: '+@VentaTotal
	End
Go

Create Proc ConsultaTotalVentaMes
(
@FechaVenta Date,
@Mensaje Varchar(50) Output
)
As Begin
Declare @VentaTotal Varchar(20)
		Set @VentaTotal = (select sum(Total) from Venta where month(FechaVenta) = month(@FechaVenta))
		Set @Mensaje='La venta total del mes es: '+@VentaTotal
	End
Go

Create Proc ConsultaTotalVentaConFiltro
(
@FechaVenta Date,
@Usuario Varchar(50),
@Mensaje Varchar(50) Output
...
As Begin
Declare @VentaTotal Varchar(20)
		Set @VentaTotal = (select sum(Total) from Venta
		inner join Usuario on Usuario.IdEmpleado = Venta.IdEmpleado
		Where Venta.FechaVenta = @FechaVenta and Usuario.Usuario = @Usuario)
		Set @Mensaje='La venta total del día de éste usuario es: '+@VentaTotal
	End
Go

Create Proc ConsultaTotalVentaConFiltroMes
(
@FechaVenta Date,
@Usuario Varchar(50),
@Mensaje Varchar(50) Output
...
Declare @VentaTotal Varchar(20)
		Set @VentaTotal = (select sum(Total) from Venta
		inner join Usuario on Usuario.IdEmpleado = Venta.IdEmpleado
		Where month(Venta.FechaVenta) = month(@FechaVenta) and Usuario.Usuario = @Usuario)
		Set @Mensaje='La venta total del mes de éste usuario es: '+@VentaTotal
	End
Go
...
As Begin
	Select IdProducto,IdCategoria,Nombre,Marca,PrecioCompra,PrecioVenta,Stock,StockMinimo,FechaVencimiento 
	From Producto
End
go

Create Proc ListadoProductos2
As Begin
...
As Begin
	Select IdUsuario, Usuario From Usuario
End
Go

Insert Producto Values(1,'Martillo','Tigre',1000,10,4.90,5.90,'12/12/2018')
Go
...
Create Proc ListadoProductosStockBajo
As Begin
	Select * From Producto Where Stock <= StockMinimo Order By Stock Asc
   End
Go
...
As Begin
	...
	Else Begin
		Insert Producto Values(@IdCategoria,@Nombre,@Marca,@Stock,@StockMinimo,@PrecioCompra,@PrecioVenta,@FechaVencimiento)
		Set @Mensaje='Registrado Correctamente.'
	End
End
Go
...
As Begin
	...
	Else Begin
		Update Producto Set IdCategoria=@IdCategoria,Nombre=@Nombre,Marca=@Marca,Stock=@Stock,StockMinimo=@StockMinimo,
...
Create Proc ActualizarStockVendido
(@IdProducto int,
@Cantidad int
)
As Begin
			...
			If(Not Exists(Select Contraseña From Usuario Where Contraseña=@Contraseña))
				...
				Else Begin
					...
					Select Usuario,Contraseña From Usuario Where Usuario=@Usuario And Contraseña=@Contraseña
							Set @Mensaje='Bienvenido/a: '+@Empleado+'.'
						End
				  End
		   End
   End
...
Create Proc MantenimientoUsuarios
@IdEmpleado Int,
@Usuario Varchar(50),
@Contraseña Varchar(50),
@Mensaje Varchar(100) Out
As Begin
	Update Usuario Set Usuario=@Usuario,Contraseña=@Contraseña Where IdUsuario=@IdEmpleado
	Set @Mensaje='Los datos se actualizaron correctamente.'
  End
Go
...
As Begin
	If(Not Exists(Select * From Empleado Where IdEmpleado=@IdEmpleado))
		Begin
		Insert Empleado Values(@IdCargo,@Dni,@Apellidos,@Nombres,@Sexo,@FechaNac,@Direccion,@EstadoCivil)
			Set @Mensaje='No se encontro el empleado con el ID:'+@IdEmpleado+'.'
		End
	Else Begin
	If(Exists(Select * From Empleado Where Dni=@Dni))
		Begin
		Delete From Empleado Where IdEmpleado=@IdEmpleado
			Set @Mensaje='El empleado fue eliminado correctamente.'
		End
	End
End
Go
...
Create proc BuscarIdEmpleadoLogueado(
@Datos Int
)
As Begin
		select IdCargo from Empleado where IdEmpleado = @Datos;
End
go 
...
As Begin
		Select E.*,C.Descripcion
		From Cargo C Inner Join Empleado E On C.IdCargo=E.IdCargo
...
As Begin
	Select IdProducto,IdCategoria,Nombre,Marca,PrecioCompra,PrecioVenta,Stock,StockMinimo,FechaVencimiento 
	From Producto
	where Nombre like @Datos +'%' or Marca like @Datos +'%' or Nombre+' '+Marca like @Datos +'%'
End
Go
...
Insert Cargo Values('Empleado')
Insert Cargo Values('Administrador')
Go
Insert Empleado Values(2,'00000000','admin','admin','M','03/18/1990','admin','S')
Go
Insert Usuario Values(1,'admin','123')
Go
